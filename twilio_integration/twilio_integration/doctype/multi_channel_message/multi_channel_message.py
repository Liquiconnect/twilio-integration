...
class MultiChannelMessage(Document):
    ...
    def get_attachment_url(self):
        ...
        file_doc = frappe.get_all(
            "File", filters={"name": self.attachment}, fields=["file_url"], limit=1
        )
        if file_doc:
    ...
    def trigger_send_mail_alert(self):
        ...
        if self.attachment:
            attachments.append({"file_url": self.attachment})
    ...
    def validate_email(self):
        ...
        self.cc = self._validate_and_format_emails(self.cc, "CC")

        self.bcc = self._validate_and_format_emails(self.bcc, "BCC")
...
def send_dynamic_whatsapp_template(self):
    # Fetch content_sid from selected template
    content_sid = frappe.db.get_value(
        "WhatsApp Template Reference",
        {"name": self.whatsapp_template, "disabled": 0},
        "content_sid",
    )

    if not content_sid:
        frappe.throw("WhatsApp template not configured properly")

    if not self.template_details:
        frappe.throw("Template variables are required")

    # Build variable dictionary
    message = {}

    for row in self.template_details:
        if not row.variable:
            continue

        key = row.variable.strip("{}")  # convert {{1}} → 1

        if not row.data:
            frappe.throw(f"Value missing for variable {row.data}")

        message[key] = row.data
